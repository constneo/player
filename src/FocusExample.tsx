      ...
      {({ focused, pressed }) => {
        return (
          <Text style={styles.pressableText}>
            {pressed ? `${props.title} pressed` : focused ? `${props.title} focused` : props.title}
          </Text>
        )
      }}
    </Pressable>
...
export default function () {
  ...
  const [isContainerFocused, setIsContainerFocused] = React.useState<boolean>(false)
    ...
    if (eventType !== "focus" && eventType !== "blur") {
      setRemoteEventLog(log =>
        logWithAppendedEntry(
          log,
          `type=${eventType}, action=${eventKeyAction !== undefined ? eventKeyAction : ""}`
        )
      )
    }
  })
  ...
  return (
    <View style={styles.container}>
      <View>
        ...
        <TVFocusGuideView
          style={[styles.containerView, isContainerFocused ? { backgroundColor: "#cccccc" } : {}]}
          onBlurCapture={(event: any) => {
            updatePressableLog(`Container captured blur event for ${event.nativeEvent.target}`)
          }}
          onFocusCapture={(event: any) => {
            updatePressableLog(`Container captured focus event for ${event.nativeEvent.target}`)
          }}
          onBlur={(event: any) => {
            updatePressableLog(
          ...
          <Text style={{ fontSize: 12 * scale }}>Container receives bubbled events</Text>
          <PressableButton title="Contained button 1" log={updatePressableLog} />
          <PressableButton title="Contained button 2" log={updatePressableLog} noBubbledEvents />
        </TVFocusGuideView>
        {/* Text Input */}
        <View
          style={styles.containerView}
          onFocus={(event: $FlowFixMe) => {
            updatePressableLog(`Focus bubbled from ${event.nativeEvent.target}`)
          }}
          onBlur={(event: $FlowFixMe) => {
            updatePressableLog(`Blur bubbled from ${event.nativeEvent.target}`)
          }}>
          <Text style={{ fontSize: 12 * scale }}>TextInput wrapped with TouchableOpacity</Text>
          <TouchableOpacity
            style={[
              styles.pressable,
              {
            ...
            <View>
              <TextInput
                ref={textInputRef}
                onFocus={(event: $FlowFixMe) =>
                  updatePressableLog(`TextInput ${event.nativeEvent.target} is focused`)
                }
                onBlur={(event: $FlowFixMe) =>
                  updatePressableLog(`TextInput ${event.nativeEvent.target} is blurred`)
                }
